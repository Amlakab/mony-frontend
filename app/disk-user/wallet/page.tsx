  ...
  const DepositForm = () => {
    ...
    const handlePaymentSubmit = async (e: React.FormEvent) => {
      ...
      if (parseFloat(amount) < 10000) {
        showMessage('Minimum deposit amount is 10,000 ETB', 'error');
        return;
      }
    ...
    const handleVerificationSubmit = async (e: React.FormEvent) => {
      ...
      setLoading(true);
      try {
        // Create game_purchase transaction with all required fields
        const payload = {
          userId: user!._id,
          amount: parseFloat(amount),
          type: 'game_purchase', // Always set to game_purchase
          reference: `GAME-${Date.now()}-${user!._id}-${transactionId}`,
          description: `Game purchase via ${paymentMethod.toUpperCase()}`,
        ...
        if (res.data.success) {
          showMessage('Game purchase request submitted successfully! It will be processed after verification.', 'success');
          ...
          showMessage('Failed to submit game purchase request', 'error');
        }
      } catch (err: any) {
        console.error(err);
        showMessage(err.response?.data?.message || 'Game purchase failed', 'error');
      } finally {
        setLoading(false);
      }
    };
        ...
        {isLoadingAccountants ? (
          ...
          <form onSubmit={handlePaymentSubmit}>
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500 mt-1">Minimum: 10,000 ETB</p>
            </div>
          ...
          {transactions.map((transaction) => (
            ...
            >
              <div className="flex items-center">
                <div className={`p-2 rounded-full ${transaction.type === 'game_purchase' || transaction.type === 'winning' ? 'bg-green-100' : 'bg-red-100'}`}>
                  {transaction.type === 'game_purchase' || transaction.type === 'winning' ? 
              ...
              <div className="flex items-center">
                <div className={`text-right mr-3 ${transaction.type === 'game_purchase' || transaction.type === 'winning' ? 'text-green-600' : 'text-red-600'}`}>
                  <p className="font-semibold">{transaction.type === 'game_purchase' || transaction.type === 'winning' ? '+' : '-'}{formatCurrency(transaction.amount)}</p>
                 <p
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <p className={`font-semibold ${selectedTransaction.type === 'game_purchase' || selectedTransaction.type === 'winning' ? 'text-green-600' : 'text-red-600'}`}>
                      {selectedTransaction.type === 'game_purchase' || selectedTransaction.type === 'winning' ? '+' : '-'}
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    {selectedTransaction.senderPhone && (
                      <div>
                        <p className="text-xs text-gray-500">
                          {selectedTransaction.type === 'game_purchase' ? 'From (Your Account)' : 'From (Platform Account)'}
                        </p>
                    ...
                    {selectedTransaction.receiverPhone && (
                      <div>
                        <p className="text-xs text-gray-500">
                          {selectedTransaction.type === 'game_purchase' ? 'To (Platform Account)' : 'To (Your Account)'}
                        </p>
